...
case "${variant}" in
    production)
        packaged_name="gpu30CudaBackend"
        display_name="production"
        ;;
    twophaseflux-validation)
        packaged_name="gpu30CudaBackend-twophaseflux-validation"
...
installed_backend="${FOAM_USER_APPBIN}/gpu30CudaBackend"
...
if [[ -n "${status_directory}" ]]; then
    ...
    } > "${status_directory}/ACTIVE_GPU30_BINARY.md"
fi
