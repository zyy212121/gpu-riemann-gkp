#!/usr/bin/env python3
"""Chunked readers/writer for legacy GPU2 and weighted GPU3 particle SoA."""
    ...
    with path.open("rb") as stream:
        ...
        if kind == "GPU2_PARTICLES_V4":
            ...
            if legacy_parcel_mass is None or not np.isfinite(legacy_parcel_mass) or legacy_parcel_mass <= 0:
                raise ValueError("legacy parcel mass is required for GPU2 V4/V5 restart")
            rows: list[list[str]] = []
            emitted = 0
            for raw in stream:
                if not raw.strip():
        ...
        if kind not in {"GPU2_PARTICLES_V5_BIN", "GPU3_PARTICLES_V1_BIN"}:
        ...
        weighted = kind == "GPU3_PARTICLES_V1_BIN"
        if not weighted:
            if legacy_parcel_mass is None or not np.isfinite(legacy_parcel_mass) or legacy_parcel_mass <= 0:
                raise ValueError("legacy parcel mass is required for GPU2 V4/V5 restart")
...
def write_gpu3_restart(
    ...
    with path.open("xb") as stream:
        stream.write(
            f"GPU3_PARTICLES_V1_BIN {total_particles} {chunk_particles}\n".encode("ascii")
        )
